...
export async function POST(request) {
  try {
    ...
    const imagePath = `/my basic programming/Ten Projects For Portifolio/CodeWIthharry Next js Project/panda/public/Pos/${file.name}`;
    await writeFile(imagePath, buffer);
    console.log(`Uploaded image saved at ${imagePath}`);

    // Read the existing JSON file (if it exists)
    const jsonPath = '/my basic programming/Ten Projects For Portifolio/CodeWIthharry Next js Project/panda/public/PosImageLinks.json'; // Specify the path to your JSON file
    let jsonData = [];
    try {
